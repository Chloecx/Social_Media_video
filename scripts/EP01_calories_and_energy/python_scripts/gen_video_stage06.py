...
OUTPUT_PATH = os.path.join(ASSETS_DIR, "Stage6_Output.mp4")

VIDEO_SIZE = (1920, 1080)
FPS = 30
SHOT_GAP = 0.5  # 镜头尾部定格缓冲时间 (0.5秒)
VALID_EXTENSIONS = [".png", ".jpg", ".jpeg", ".PNG", ".JPG", ".JPEG"]

# ---------------------------------------------------------
# 2. Scene 6 Shot 与素材 mapping 配置
# ---------------------------------------------------------
shots_data = [
    {
        "shot_id": "Shot6-1",
        "audio_file": "Shot6-1.mp3",
        "frames": ["Shot6-1_01", "Shot6-1_02"],
        "auto_align": True,
        # 以 "don't" (Don't forget to like and subscribe) 为切图卡点
        "target_keywords": ["don't", "dont"]
    }
]
...
print("🎬 开始处理 Scene 6 视频合成...", flush=True)
...
for shot in shots_data:
    ...
    for idx, base_name in enumerate(frame_names):
        ...
        # 镜头最后一张图片增加 0.5s 停留缓冲，方便留出片尾元素展示时间
        if idx == num_frames - 1:
            dur += SHOT_GAP
...
if compiled_shot_clips:
    ...
    print(f"🚀 开始渲染 Scene 6 最终视频至: {OUTPUT_PATH}", flush=True)
    ...
    print("✅ Scene 6 视频制作完成！输出文件：Stage6_Output.mp4", flush=True)
