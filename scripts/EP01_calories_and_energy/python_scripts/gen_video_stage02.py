...
OUTPUT_PATH = os.path.join(ASSETS_DIR, "Stage2_Output.mp4")

VIDEO_SIZE = (1920, 1080)
FPS = 30
SHOT_GAP = 0.5  # 镜头间静止画面定格缓冲时间 (0.5秒)
VALID_EXTENSIONS = [".png", ".jpg", ".jpeg", ".PNG", ".JPG", ".JPEG"]

# ---------------------------------------------------------
# 2. Scene 2 Shot 与素材 mapping 配置
# ---------------------------------------------------------
shots_data = [
    {
        "shot_id": "Shot2-1",
        "audio_file": "Shot2-1.mp3",
        "frames": ["Shot2-1_01"],
        "auto_align": False
    },
    {
        "shot_id": "Shot2-2",
        "audio_file": "Shot2-2.mp3",
        "frames": ["Shot2-2_01", "Shot2-2_02"],
        "auto_align": True,
        # 画面1截至 291 / hours，画面2从 more (than 12 full days) 开始
        "target_keywords": ["291", "more"]
    },
    {
        "shot_id": "Shot2-3",
        "audio_file": "Shot2-3.mp3",
        "frames": ["Shot2-3_01"],
        "auto_align": False
    },
    {
        "shot_id": "Shot2-4",
        "audio_file": "Shot2-4.mp3",
        "frames": ["Shot2-4_01"],
        "auto_align": False
    },
    {
        "shot_id": "Shot2-5",
        "audio_file": "Shot2-5.mp3",
        "frames": ["Shot2-5_01", "Shot2-5_02"],
        "auto_align": True,
        # 画面1截至 2.5 / miles，画面2从 or (roughly 4.0 km) 开始
        "target_keywords": ["miles", "or"]
    }
]
    ...
    for kw in target_keywords:
        ...
        for item in word_timestamps:
            if kw_clean in item["word"] or item["word"] in kw_clean:
                # 若为切出词 (如 291/miles) 选 end；若为切入词 (如 more/or) 选 start
                split_time = item["start"] if kw_clean in ["more", "or"] else item["end"]
                print(f"  └─ 🎯 识别到卡点关键字 [{item['word']}]，切图时间点: {split_time:.2f}秒", flush=True)
                break
        if split_time is not None:
            break

    # 若成功找到分割点，计算 2 张图各自的时长；否则均分
    if split_time is not None and 0.5 < split_time < (total_duration - 0.5):
        dur1 = split_time
        dur2 = total_duration - split_time
        return [dur1, dur2]
    else:
...
print("🎬 开始处理 Scene 2 视频合成...", flush=True)
...
if compiled_shot_clips:
    ...
    print(f"🚀 开始渲染 Scene 2 最终视频至: {OUTPUT_PATH}", flush=True)
    ...
    print("✅ Scene 2 视频制作完成！输出文件：Stage2_Output.mp4", flush=True)
